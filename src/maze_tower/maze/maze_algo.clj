  ...
  (lazy-seq
    (when coord
      (let [cell (g/grid-cell grid coord)
            ...
            ;;_ (println "cell:" cell "neighbors:" neighbors)
            next-grid (if (empty? neighbors)
                        grid
                        (do
                          ;;(println "link cell " cell (rand-nth neighbors))
...
(def updates-only
  (comp (dedupe) (filter #(or (g/changed? %) (g/new? %)))))
  ...
  (let [algorithm #(last ((algorithm-functions name) %))
    ...
    (fn [grid]
      (let [maze (assoc (algorithm grid) ::g/changed-cells (into #{} (g/grid-coords grid)))
            start (get options :distances [0 0])
            end (get options :path-to [(dec (::g/rows grid))
                                       (dec (::g/cols grid))])
